...
![](https://github.com/lanhuongnguyen276/NDH802/blob/master/Spring%25202022/Exercices/pr1.jpg?raw=true)

#### Prediction

After fitting the model, we can write it as:

$\hat{y} = -3.5695 + 3.6954*x_{new}$

Prediction (in this course) is simply plugging in the new x to the model to find the $\hat{y}$.

```{r}
#the more systematic way. normally, we fit the model many times, and we may want to automatize the process like this
b0 = as.numeric(coef(mod)[1])
b1 = as.numeric(coef(mod)[2])
...
$HEI = b_0 + b_1*DailyCost + \varepsilon$
...
$HEI = b_0 + b_1*BMI+ b_5*female + \varepsilon$

```{r}
mod_ex = lm(HEI2005 ~ BMI + female, data = hei)
summary(mod_ex)

{#you are not expected to code the below plot. only for visualization
plot(HEI2005 ~ BMI, data = hei, col = "gray")
abline(a = coef(mod_ex)[1], b = coef(mod_ex)[2], col="red") #male
abline(a = coef(mod_ex)[1] + coef(mod_ex)[3], b = coef(mod_ex)[2], col="blue") #female
}
```

a\. Here we are asked to fit the basic linear model:

$HEI = b_0 + b_1*docbp + b_2*waistper + b_3*BMI+ b_4*sroverweight + b_5*female + b_6*age + b_7*daycode + \varepsilon$
...
$HEI = b_0 + b_1*docbp + b_2*waistper + b_3*BMI+ b_4*sroverweight + b_5*female + b_6*age + b_7*daycode + b_8*immigrant + \varepsilon$
...
$HEI = b_0 + b_1*docbp + b_2*waistper + b_3*BMI+ b_4*sroverweight + b_5*female + b_6*age + b_7*daycode + b_8*single+ \varepsilon$
...
$HEI = b_0 + b_1*docbp + b_2*waistper + b_3*BMI+ b_4*sroverweight + b_5*female + b_6*age + b_7*daycode + b_8*fsp+ \varepsilon$
...
-   **Will I be asked to fit an OLS regression?** It is possible.
...
-   **Do I need to be able to interpret the OLS regression model summary from R?** Yes, especially the R-squared, p-values, magnitude and sign of the coefficients.
