...
-   **The text**: where you write generally everything that is not the other three components, like what you're reading. You can format the text, e.g., *italics* and *italics* , **bold** and **bold**, superscript^2^, ~~strikethrough~~.
-   **The code chunks**: where the magic happens in RMarkdown. More on this later.
-   **Inline code**: you can use inline code to refer to some values and it changes when your values change, for example `r Sys.Date()`. Notice what you see in the RMarkdown file, and what you'll see in the final document (I think we'll do this in the live session). In this case, `Sys.Date()` prints the date you run the code, i.e., if you run this file in different dates, it prints out different values. As another example, notice how I integrate the `page` result in the last section. This can come in handy when you write up your assignments or other reports.

## Some words about the assignments

You will be asked to use RMarkdown to write up your assignments because:

-   you can write your thoughts in words, code, print out the results, visualize your data, and much more, all in one place.
-   you don't have to copy your results from, e.g., Excel and paste them into, e.g., Word. Why do manual labor when you can automate things (and save that time for something else)?
...
-   Finally, I really hope it helps developing a new habit and forever enhance your workflow.

This can also happen. When your code cannot run or it produces errors because of, e.g., what I consider a typo, but I can see your (correct) thought process in there, you will get full score. It's because (1) your understanding is what matters most and (2) I am nice, in general. However, it doesn't work the other way around. That is, the results (correct or not) without code/justifications won't be graded. I don't care about the mean. I care if you can compute the mean.
...
For example, assume Huong gets 9 pt on Assignment 1, 10 pt on Assignment 2 (😎), and 7 pt on Assignment 3. The mean of Huong's three assignment is:
...
Now let's create another code chunk, copy and paste this `read.csv("https://raw.githubusercontent.com/lanhuongnguyen276/NDH802/master/Preps/Before_Live_1/salaries.csv")` into your chunk, then Ctrl(Windows)/Cmd(Mac) + Enter. What do you see?

Similar to giving 2+2 a shorter name "x", you can name your data table. Now try paste it to your code chunk `salaries = read.csv("https://raw.githubusercontent.com/lanhuongnguyen276/NDH802/master/Preps/Before_Live_1/salaries.csv")` , then run it.

This is loading your data into R Environment. From now on (until you restart your R session), whenever you type `salaries` in code chunks, R will understand you refer to the above data.
