...
# Probability theory
Let $A$ be an event. To calculate $N_A$ in base R, the simpliest way is to use ` table() `. For example, assume we want to know the number of BE ($N_{BE}$) and RM ($N_{RM}$) students in our data, we can
```{r}
...
(2) Make a boxplot for `idealpay`.
(3) Make a boxplot for `idealpay`, for RM and BE students.
(4) Compute the mean and variance of `fairpay`.
(4) Compute the mean and variance of `fairpay` of the female students.
(5) Compute the mean and variance of `fairpay` of the students who are female and belong to RM program.
(6) How many male students and female students do we have in the data set? Formally, compute $N_{male}$ and $N_{female}$
(7) How many *students who are female and belong to RM program* do we have in the data set? compute $N_{female \cap RM}$
