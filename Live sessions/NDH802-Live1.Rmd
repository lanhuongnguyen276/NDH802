...
There are numerious way to select a column. The three most basic way are:
```{r, results='hide'}
salaries$sal_expected
salaries[,"sal_expected"] #here you refer to the column by the columns names
salaries[,1] #here you refer to the column by the column position from the left ie 1 is the first column from the left
```
To select more than 1 column:
```{r, results='hide'}
salaries[,c("sal_expected", "fairpay", "program")] #here you refer to the column by the columns names
salaries[,c(1,3,10)]  #here you refer to the column by the column position from the left
```

If your columns are next to each other, you can do this.
```{r, results='hide'}
salaries[,1:4]
```

**When to use which?**  
It's all about your personal preference. I introduce several ways to give you options, **not** to confuse you. You can totally explore and see which you're more comfortable with, then stick with one. This is also applicable for most of other commands.

### Select rows from a dataframe
There are numerious way to select a row. The most basic way (with base R) are:
```{r, results='hide'}
salaries[salaries$program == "BE",]
salaries[salaries$whatisyourgender1isfemale == 1,]
```

### Let's take a closer look
Assume you want to explore the salary expectation from the students, which is the variable `sal_expected`.
Two of my most frequent used methods are histogram and box plot.

Let's first try plotting a **histogram**. If you're like "a histo what?", no worries, take a quick look [here](https://www.youtube.com/watch?v=qBigTkBLU6g), then come back.
```{r}
hist(salaries$sal_expected)
```
...
```{r, echo=FALSE, results='hide'}
# boxplot(
#   sal_expected / 1000 ~  whatisyourgender1isfemale + program,
#   data = salaries,
#   ylim = c(0, 120),
#   col = c("darkcyan", "lightblue"),
#   horizontal = TRUE
# )
```
...
### Can I calculate the means and SD of several variables at the same time?
Hint: you can use ```colMeans```. But the below code chunk doesn't work. Based on the error message, what do you think is the cause and how to fix it?
```{r}
#colMeans(salaries, na.rm = TRUE)
```
More advancedly, you can use loop. This is more efficient, yet tricker to work with.
```{r}
lapply(salaries[,1:4], mean, na.rm = TRUE)
lapply(salaries[,1:4], sd, na.rm = TRUE)
```
...
mean(salaries[salaries$program == "RM","sal_expected"], na.rm = TRUE)
#aggregate(sal_expected ~ program, data = salaries, FUN = mean)
```

### Can I calculate the mean of salary expected of the female RM student?
```{r}
mean(salaries[salaries$program == "BE" &
              salaries$whatisyourgender1isfemale == "1", "sal_expected"], na.rm = TRUE)
# aggregate(sal_expected ~ program + whatisyourgender1isfemale,
#           data = salaries,
#           FUN = mean)
```

# Probability theory
Let $A$ be an event. To calculate $N_A$, I think the simpliest way is to use  ` table() `. For example, assume we want to know the number of BE ($N_{BE}$) and RM ($N_{RM}$) students in our data, we can
```{r}
...
* if RM and BE are mutually **exclusive/collectively exaustive** events?
