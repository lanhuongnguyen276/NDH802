...
**(a) What is the probability that the cost of building a home will be between \$460,000 and \$540,000? That is, $P(460,000 < X < 540,000)$ where $X$ denotes the cost?**  
What we want to compute is the *blue* area. To do that in base R, one way is to compute the grey area (which is $F(b)$ in Lecture 4 - slide 6), then subtract the pink area (which is $F(a)$ in Lecture 4 - slide 6) from it.
...
par(mfrow = c(3,1))
shade.norm(from = mu-4*sigma, to = 540,sigma=sigma,mu=mu,tail="middle", show.dist = F, show.p = F)
shade.norm(from = mu-4*sigma, to = 460,sigma=sigma,mu=mu,tail="middle", show.dist = F, show.p = F, shade.col = "pink3")
shade.norm(from = 460, to = 540,sigma=sigma,mu=mu,tail="middle", show.dist = F, show.p = F, shade.col = "lightblue3")
}
```
...
The graph is, again, for illustration purpose, you don't need to learn to code it.  
Notice the lavendar area is 0.95.  
```{r, fig.height=3, fig.width=6, fig.align='center'}
b = qnorm(0.025, mean = mu, sd = sigma)
c = qnorm(0.975, mean = mu, sd = sigma)
shade.norm(from = b, to = c, sigma=sigma, mu=mu, tail="middle", show.dist = F, show.p = F, shade.col = "lavender")
```

***NOTE***  
- In (a), we compute the **area** below the curve (the grey/pink/blue area) which characterizes the **probability**.  
